...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    // Must have either html or template
    if (!html && !template) {
      return new Response(
    ...
    // Generate HTML from template if provided, otherwise use html
    let finalHtml = html || ''
    if (template) {
      if (!data) {
        return new Response(
          JSON.stringify({ error: 'Missing required field: data (required when using template)' }),
          { status: 400, headers: { 'Content-Type': 'application/json' } }
        )
      }
